...
namespace remote {
    ...
    void MapModuleMemoryRegion::find(Handle handle, const char* data, size_t size, std::vector<size_t> &addresses)
    {
        char buffer[FINDPATTERN_CHUNKSIZE];
        size_t chunksize = sizeof(buffer);
        size_t totalsize = this->end - this->start;
        ...
        while (totalsize) 
        {
            ...
            if (handle.Read((void*) readaddr, buffer, readsize)) 
            {
                ...
                {
                    size_t matches = 0;
                    while (buffer[b + matches] == data[matches]) 
                    {
                        matches++;
                        if (matches == size) addresses.push_back(readaddr + b);
                    }
                }
            }
    ...
    void MapModuleMemoryRegion::print(Handle handle, void* address, unsigned long size)
    {
        char buffer[size];

        size_t chunksize = sizeof(buffer);
        size_t readaddr = (size_t)address - ((size_t)size)/2;

        bzero(buffer, chunksize);

        if (handle.Read((void*) readaddr, buffer, size)) 
        {
